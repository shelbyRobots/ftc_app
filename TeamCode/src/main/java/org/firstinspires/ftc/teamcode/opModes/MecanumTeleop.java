    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        while (opModeIsActive())
        {
            ...
            final double direction = Math.atan2(lr_x, fb_y) +
                                     (fieldAlign ?
                                        Math.toRadians(robot.getGyroFhdg()) : 0.0);
            ...
            if(useSetVel)
            {
                ...
                ((DcMotorEx)robot.lfMotor).setVelocity(lf*dps, AngleUnit.DEGREES);
                ((DcMotorEx)robot.rfMotor).setVelocity(rf*dps, AngleUnit.DEGREES);
                ((DcMotorEx)robot.lrMotor).setVelocity(lr*dps, AngleUnit.DEGREES);
                ((DcMotorEx)robot.rrMotor).setVelocity(rr*dps, AngleUnit.DEGREES);
